...
Texture2D itemIcon ; //The Icon.
bool canGet = true; //If we can pick up the Item.
string itemType ; //This will let us equip the item to specific slots. Ex: Head, Shoulder, or whatever we set up. If the item is equipment (or weapon) this needs to match a slot to work properly.
bool stackable = false; //Is it stackable? If yes then items with the same itemType will be stacked.
int maxStack = 20; //How many Items each stack can have before creating a new one. Remember that the Items that should be stacked should have the same itemType.
int stack = 1; //This is how many stack counts this Item will take up.
bool isEquipment = true; //Can the Item be equipped? This includes weapons.
bool isAlsoWeapon = false; //Is the Item also a Weapon? This only works with isEquipment set to true.
...
//Picking up the Item.
void PickUpItem (RInventory inv)
{
	playersinv = inv;
	bool getit=true;
...
void MoveMeToThePlayer(Transform itemHolderObject )
{
	...
	GetComponent<RItem>().enabled = false;
...
void DropMeFromThePlayer(bool makeDuplicate)
{
	if (makeDuplicate == false) //We use this if the object is not stacked and so we can just drop it.
	{
		...
		GetComponent<RItem>().enabled = true;
	...
	else //If the object is stacked we need to make a clone of it and drop the clone instead.
	{
		...
		clone.GetComponent<RItem>().enabled = true;
