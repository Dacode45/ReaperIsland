...
class RInventoryEditor extends EditorWindow {
	...
	@MenuItem ("Window/RInventory %i")
	
	static function Init ()
	{
		// Get existing open window or if none, make a new one:
		var window = ScriptableObject.CreateInstance.<RInventoryEditor>();
		window.title = "RInventory";
		window.Show();
	}
	...
	function InventorySetUp ()
	{
		...
		if (selectedObject.FindChild("RGUI") != null)
		{
			Debug.LogError("An object called 'RGUI' was already found on " + selected + ". If the Inventory has previously been set up on the selected object (" + selected + ") then make sure to delete it by pressing 'Remove Inventory'.");
			return;
		}
		
		var invObject = new GameObject ("RGUI");
		...
		invObject.AddComponent(RInventory);
		invObject.AddComponent(RInventoryDisplay);
		
		if (includeCSheet)
		{
			invObject.AddComponent(RICharacter);
			
			if (selectedObject.GetComponent(RICharacter) != null)
			{
				Debug.LogWarning("The Character Sheet (RCharacter) script was found on the selected object (" + selected + "). Make sure to delete all traces of any previously installed Inventory. There should only be an 'Inventory', 'InventoryDisplay' and 'Character' script on an Inventory gameobject parented to the Player. The Inventory was still set up though.");
			}
		}
		
		//Set up the cSheet variables
		var csheet = invObject.GetComponent(RICharacter);
		
		if (cSheetGUI != null)
		{
			csheet.invSkin = cSheetGUI;
		}
		else
		{
		...
		var invDisp = invObject.GetComponent(RInventoryDisplay);
	...
	function InventoryDelete ()
	{
		...
		if (selectedObject.FindChild("RGUI") != null)
		{
			DestroyImmediate(selectedObject.FindChild("RGUI").gameObject);
		}
		
		if (selectedObject.GetComponent(RICharacter) != null)
		{
			DestroyImmediate(selectedObject.gameObject.GetComponent(RICharacter));
		}
		
		if (selectedObject.GetComponent(RInventory) != null)
		{
			DestroyImmediate(selectedObject.gameObject.GetComponent(RInventory));
		}
		
		if (selectedObject.GetComponent(RInventoryDisplay) != null)
		{
			DestroyImmediate(selectedObject.gameObject.GetComponent(RInventoryDisplay));
		}
